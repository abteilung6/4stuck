        ...
        try:
            ...
            if user.color:
                return {
                    "success": True,
                    "color": user.color,
                    "message": "User already has color assigned"
                }
            ...
            for attempt in range(len(self.color_scheme)):
                ...
                if available_color and available_color != self.fallback_color:
                    ...
                    if conflict:
                        ...
                        db.commit()
                        continue
                    logger.info(f"Assigned color {available_color} to user {user_id} in team {team_id} (attempt {attempt+1})")
                    return {
                        "success": True,
            ...
            return {
                "success": False,
                "color": self.fallback_color,
                "message": f"Error assigning color: {str(e)}"
            }
