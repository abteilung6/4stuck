...
class GameSession(BaseModel):
    ...
    started_at: Optional[datetime] = Field(description="When the game started (active status)")
    ended_at: Optional[datetime] = Field(description="When the game ended (finished status)")
    survival_time_seconds: Optional[int] = Field(description="How long the team survived in seconds")
    countdown_duration: Optional[int] = Field(description="Countdown duration in seconds before game starts")
...
class GameState(BaseModel):
    ...
    puzzles: Optional[List[Any]] = Field(description="Current puzzles for all players")
    timestamp: datetime = Field(description="When this game state was captured")
    countdown_remaining: Optional[int] = Field(description="Seconds remaining in countdown (if in countdown status)")
    next_point_decay: Optional[datetime] = Field(description="When the next point decay will occur")
...
class GameConfig(BaseModel):
    ...
    max_team_size: Optional[int] = Field(description="Maximum number of players per team")
    countdown_duration: Optional[int] = Field(description="Countdown duration in seconds before game starts")
...
class GameResult(BaseModel):
    ...
    puzzles_solved_per_player: Optional[Dict[str, Any]] = Field(description="Number of puzzles solved by each player")
    points_given_per_player: Optional[Dict[str, Any]] = Field(description="Total points given by each player to teammates")
    points_received_per_player: Optional[Dict[str, Any]] = Field(description="Total points received by each player from teammates")
